...
import { useSharedState } from '@features/tabs/SharedContex';
...
const LoactionHeader: FC = () => {
    const { scrollyGloabl } = useSharedState();
    const { styles } = useStyles(homeStyles);
    const textColor = '#fff';
  
    const opacityFadingStyles = useAnimatedStyle(() => {
      const opacity = interpolate(scrollyGloabl.value, [0, 80], [1, 0]);
      return {
        opacity: opacity,
      };
    });
      ...
      <Animated.View style={opacityFadingStyles}>
        ...
        <View style={styles.flexRowBetween}>
          ...
          <View>
            ...
            <CustomText color={textColor} fontFamily='Okra-Medium'>Pune,Maharashtra</CustomText>
          </View>
          <View style={styles.flexRowGap}>
